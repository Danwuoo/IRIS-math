...
- **Benchmark Family Policy (`benchmark_family_policy/v1`):** executable family object that declares allowed tiers, train-visible units, homology axes, source-lineage firewalls, held-out sources, tuning visibility, derivative-family refs, and forbidden uses.
...
- **Control State:** the singleton State IR control record with symbolic control actions, explicit budget, uncertainty, and escalation state.
...
- **Verifier Evidence:** auditable local, global, counterexample, or formal-check signals that can influence validity judgment, credit routing, or recovery.
