...
def test_s4_v2_fails_when_isolation_decreases_and_leakage_increases() -> None:
    status, reasons, details = evaluate_s4_status_v2(
        current_concept_breakdown={
            "concepts": [
                {"concept_id": "Copy", "concept.isolation_score": 0.40, "concept.leakage_score": 0.45}
            ]
        },
        baseline_concept_breakdown={
            "concepts": [
                {"concept_id": "Copy", "concept.isolation_score": 0.90, "concept.leakage_score": 0.10}
            ]
        },
        tolerances=Tolerances(concept_isolation_delta_epsilon=0.01, concept_leakage_delta_epsilon=0.01),
        phase="D",
    )
    assert status == "FAIL"
    assert reasons
    assert any(item["metric"] == "concept.isolation_score" for item in details)
    assert any(item["metric"] == "concept.leakage_score" for item in details)


def test_s5_v2_fails_when_pair_invariance_regresses() -> None:
    status, reasons, details = evaluate_s5_status_v2(
        current_paired_diff={"pair_count": 4, "paired.asymmetry_rate": 0.25, "paired.invariance.gap": 0.20},
        baseline_paired_diff={"pair_count": 4, "paired.asymmetry_rate": 0.00, "paired.invariance.gap": 0.00},
...
def test_s3_v2_uses_taxonomy_histogram_drift() -> None:
    status, reasons = evaluate_s3_status_v2(
        failure_profile_diff={
            "status": "PASS",
            "failure_profile.l1_distance": 0.20,
            "failure_profile.kl_divergence": 0.20,
        },
        tolerances=Tolerances(failure_profile_kl_epsilon=0.01),
        phase="D",
    )
    assert status == "FAIL"
    assert reasons
...
def test_run_phase_d_gate_reproduces_concept_breakdown_hash(tmp_path: Path) -> None:
    ...
    _write_json(
        tmp_path / "conceptarc" / "Rotate" / "rotate_a.json",
        {
            "train": [{"input": [[2, 2]], "output": [[2, 2]]}],
    ...
    context = GateContext(
        phase="D",
        baseline_id="phase-d-v1",
        tolerance_profile_id="phase-d-default",
    ...
    baseline_dir = tmp_path / "baseline_phase_d_v1"
    ...
    concept_a = json.loads((tmp_path / "report_a" / "concept_breakdown.json").read_text(encoding="utf-8"))
    concept_b = json.loads((tmp_path / "report_b" / "concept_breakdown.json").read_text(encoding="utf-8"))
    assert concept_a["artifact_hash"] == concept_b["artifact_hash"]
