...
def main() -> int:
    ...
    parser.add_argument(
        "--baseline-report-dir",
        type=Path,
        default=Path("artifacts/baselines/phase-e-v1"),
    )
    parser.add_argument(
        "--freeze-baseline",
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Return non-zero when gate status is FAIL.",
    )
    args = parser.parse_args()

    if str(args.phase).strip().upper() != "E":
        raise SystemExit("Phase E gate accepts only --phase E.")

    if not args.freeze_baseline and not Path(args.baseline_report_dir).exists():
        raise SystemExit(
            "Baseline report dir is required in hard-fail mode. "
            "Use --freeze-baseline for first baseline creation."
        )
    ...
    fallback_suites = [
        suite_name
        for suite_name, payload in (("S1", s1_output), ("S2", s2_output), ("S2M", s2_mounted_output))
    ...
    report_path = str((args.output_dir / "phase_e_gate_report.md").resolve())
    print(
        json.dumps(
            {
    ...
    gate_pass = str(summary.get("regression.status", "FAIL")).upper() == "PASS"
    if args.hard_fail:
